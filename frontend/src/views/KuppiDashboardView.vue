  ...
  <div class="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12 font-sans bg-[#F8FAFC] min-h-screen px-4 md:px-8 py-8">
    ...
    <div class="mb-8 bg-white p-6 md:p-8 rounded-[24px] shadow-sm border border-slate-200">
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 class="text-[28px] font-extrabold text-[#111827] tracking-tight mb-2">
            Kuppi Tutor Dashboard
          </h1>
          <p class="text-[15px] text-[#4B5563] font-medium flex items-center gap-3">
            Welcome back, <span class="font-bold text-[#1f2432]">{{ tutorName }}</span>!
      ...
      <div class="xl:col-span-2 space-y-8">
        ...
        <div class="bg-white rounded-[24px] p-6 md:p-8 border border-slate-200 shadow-sm">
          <div class="flex items-center justify-between mb-6">
            ...
            <button class="bg-[#2563EB] hover:bg-blue-700 text-white text-[13px] font-bold py-2 px-4 rounded-xl shadow-sm transition-colors flex items-center gap-2">
          ...
          <div class="space-y-4">
            <div v-for="session in activeSessions" :key="session.id" class="flex flex-col sm:flex-row justify-between p-5 bg-slate-50 border border-slate-100 rounded-[16px] hover:border-blue-200 transition-colors group">
              <div>
                ...
                <h3 class="font-bold text-[#1f2432] text-[16px] leading-tight group-hover:text-[#2563EB] transition-colors">{{ session.title }}</h3>
              </div>
              <div class="mt-4 sm:mt-0 flex gap-2 sm:self-center">
                ...
                <button class="bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-2 px-5 rounded-xl text-[13px] transition-colors shadow-sm flex items-center gap-2">
        ...
        <div class="bg-white rounded-[24px] p-6 md:p-8 border border-slate-200 shadow-sm">
          <h2 class="text-[18px] font-bold text-[#111827] flex items-center gap-2 mb-2">
          ...
          <div class="border-2 border-dashed border-slate-300 rounded-[20px] p-10 flex flex-col items-center justify-center bg-slate-50 hover:bg-blue-50/50 hover:border-blue-400 transition-colors cursor-pointer group">
            <div class="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-sm mb-4 group-hover:scale-110 transition-transform">
              <svg class="w-8 h-8 text-[#2563EB]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
            </div>
            <h3 class="text-[#1f2432] font-bold text-[15px] mb-1">Click to upload or drag and drop</h3>
      ...
      <div class="xl:col-span-1 space-y-8 flex flex-col">
        ...
        <div class="bg-white rounded-[24px] p-6 border border-slate-200 shadow-sm relative overflow-hidden">
          ...
          <div class="bg-slate-50 border border-slate-100 rounded-xl p-4">
            <div class="flex justify-between items-center text-[13px] font-bold text-[#1f2432] mb-4">
            ...
            <div class="grid grid-cols-7 gap-1 text-center text-[12px] font-medium text-slate-700">
              ...
              <!-- Active Day -->
              <div class="p-1.5 bg-[#2563EB] text-white font-bold rounded cursor-pointer shadow-sm shadow-blue-200">8</div>
        ...
        <div class="bg-white rounded-[24px] p-6 border border-slate-200 shadow-sm flex-1">
          <h3 class="font-bold text-[#111827] text-[16px] mb-5 flex items-center justify-between">
          ...
          <div class="space-y-3">
            <div v-for="req in pendingRequests" :key="req.id" class="p-3 bg-amber-50/50 border border-amber-100 rounded-xl">
              <div class="flex justify-between items-start mb-1">
                <h4 class="font-bold text-[#1f2432] text-[13px] leading-tight">{{ req.topic }}</h4>
        ...
        <div class="bg-white rounded-[24px] p-6 border border-slate-200 shadow-sm">
          <h3 class="font-bold text-[#111827] text-[16px] mb-4 flex items-center gap-2">
          ...
          <ul class="space-y-2">
            <li v-for="attendee in recentAttendees" :key="attendee.id" class="flex items-center justify-between p-2 rounded-lg hover:bg-slate-50 transition-colors">
              <div class="flex items-center gap-3">
                ...
                <span class="text-[13px] font-medium text-[#1f2432]">{{ attendee.name }}</span>
              </div>
              <div :class="attendee.present ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'" class="text-[10px] font-bold px-2 py-0.5 rounded-full">
